...
Stosowanie algorytmów uczenia maszynowego w celu odnajdywania wartości w danych, automatyzacji zadań i usprawniania procesów biznesowych staje się coraz bardziej powszechne. Do przykładów użycia metod \textit{machine learningowych} zaliczyć można dostosowywanie wyników wyszukiwań internetowych, filtrowanie i personalizacja powiadomień w mediach społecznościowych, rekomendacje produktów i reklama internetowa, ale również rozpoznawanie dźwięku, identyfikowanie obiektów na obrazach i tłumaczenie tekstu. W tym rozdziale opisane zostaną techniki stosowane do ostatnich z wymienionych przypadków - głębokie sieci neuronowe (ang. \textit{deep neural networks}).

\subsubsection{Sieci neuronowe}

% o historii
Wraz z nadejściem elektroniki i komputerów, badacze rozpoczęli podejmować próby replikacji ludzkich procesów myślowych na systemy logiczne. Choć pierwsze kroki w tej dziedzinie zostały podjęte przez \citet{mcculloch1943} oraz \citet{hebb1949}, istotny z punktu widzenia uczenia maszynowego przełom został dokonany w pracy \citet{rosenblatt1957}, gdzie po raz pierwszy wprowadzono koncept perceptronu - algorytmu, który miał pozwolić na poprawne rozpoznawanie obrazów. Perceptron jest jednym z najbardziej podstawowych przykładów sieci neuronowych, jego schemat został przedstawiony na rysunku \ref{fig:perceptron}.
...
Pomimo obiecujących początków i optymizmu towarzyszącemu nowemu odkryciu, badania nad perceptronami zostały w dużej części porzucone za sprawą publikacji \citet{minsky1969}. Nakreślili oni granice możliwości perceptronów jako systemu uczącego się. W szczególności udowodnili, że z jego użyciem można
