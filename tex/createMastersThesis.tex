...
Algorytm uczenia perceptronu w $T$ iteracjach dla $N$ obserwacji uczących i $M$ wag przedstawiony został w Algorytm \ref{alg:perceptron}.
...
Jednym z pierwszych przykładów architektury, którą można uznać za wielowarstwową sieć neuronową jest zaprojektowany w pracach \citet{ivakhnenko1965} oraz \citet{ivakhnenko1967} system będący reprezentantem rodziny metod GMDH (ang. \textit{Group Method of Data Handling}). Jego charakterystycznymi cechami było wykorzystanie wielomianów Kolmogorova-Gabora jako funkcji aktywacji i użycie zbioru treningowego do uczenia i rozbudowywania kolejnych warstw sieci, by następnie przycinać je z wykorzystaniem zbioru walidacyjnego.
...
Istotą RMSProp jest wprowadzenie ruchomej średniej kwadratu gradientu dla każdej z wag, tak, by w każdej kolejnej partii zbioru danych dzielić przez jej pierwiastek kwadratowy, zamiast dzielenia przez wartość gradientu (Równanie \ref{}).
...
Ruch koneksjonizmu utrzymywał zainteresowanie środowisk naukowych i biznesowych tylko do połowy lat 90. Nieproporcjonalne oczekiwania co do możliwości sztucznych sieci neuronowych oraz pojawienie się innych, bardziej obiecujących metod sprawiły, że inwestorzy wycofali swoje wsparcie.

%+generalizowanie(weight decay, weight sharing, early stopping, model averaging(ensemble), bayesian fitting on neural nets, dropout, pre-training

%convnets - handwritten digits lecun 1980s (LeNet), pooling (problem w poolingu: tracisz dokładną informację o położeniu/względnym położeniu elementów)
% But it still prejudices the network towards a particular way of solving the problem that we had in mind. We have an idea about how to do object recognition by gradually making bigger and bigger features. And by replicating these features across space. And we force the network to do it that way. T
% daleko do wykrywania obrazu sądzono, bo więcej klas, reprezentacja 2d rzeczywistości 3d, więcej pikseli,

%creating training data hofman&tresp, 1993

%rectified linear units

%labelling bias by humans

Aktualne zainteresowanie sieciami neuronowymi zostało zapoczątkowane jako skutek badań nad modelami o skomplikowanej architekturze (\citet{hintonetal2006}; \citet{bengio2007a}). Wtedy też tego typu architektury sieci neuronowych otrzymały określenie \textit{głębokich}. Miało na celu podkreślenie faktu wykorzystywania wielu ukrytych warstw, co dotychczas nie było często eksplorowanym kierunkiem badań z powodu niedostatecznej mocy obliczeniowej. Istotność tej cechy w sieciach neuronowych jest podkreślana w pracach \citet{bengio2007b} i \citet{bengio2011}.
...
\bibitem[Wan (1993)]{wan1993} Wan, E.A., 1993. Time series prediction by using a connectionist network with internal delay lines. In SANTA FE INSTITUTE STUDIES IN THE SCIENCES OF COMPLEXITY-PROCEEDINGS VOLUME- (tom 15, s. 195-195). Addison-Wesley publishing co.
